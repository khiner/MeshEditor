...
**Why exempt from scaling.** Modal data describes an object at one physical size and rescales with it, but a finish does not: a scaled-up polished sphere is still polished. Contact position and sweep speed are geometric and do transform. The exemption covers the microscale parameters only. Mesoscale relief is bound to texture coordinates rather than to an absolute length, so it scales with the node like the geometry it is painted on, and a scaled-up tiled floor has larger tiles to both eye and ear.
