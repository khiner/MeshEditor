...
This extension defines sound from the bodies and contact events described by `KHR_physics_rigid_bodies`. It stores precomputed modal models, their acoustic materials, and surfaces for sustained contact. Models, materials, and surfaces are document-level resources instanced by nodes.
...
An implementation supporting impact excitation and synthesis alone is conformant. [Sustained Contact](#sustained-contact) contains its acoustic surfaces, contact state, and force requirements.
...
All three arrays are optional. An asset may define surfaces without models, allowing a body to affect contact sound without radiating its own modes.
...
When the node also has a `KHR_physics_rigid_bodies` dynamic rigid body, that body's mass properties are authoritative for contact dynamics. This extension's `massProperties`, if present, MUST be ignored. Using the simulated mass keeps audible impacts consistent with visible motion. Authored values SHOULD agree. This extension's `massProperties` apply to kinematic and static bodies because those physics bodies specify no finite mass.
...
Materials serve two roles. A modal model references its derivation parameters. An acoustic surface references the elastic constants used for contact stiffness. A non-radiating body such as a floor may therefore define a material without a model. `alpha` and `beta` apply only to modal models.
...
A conformant audio renderer synthesizes each model instance's response to contact events as follows. The source signal is far-field pressure from compact-source surface motion. Directivity, distance, occlusion, and reverberation are outside this definition. An implementation MAY add a radiation or propagation model. Without one, it SHOULD use a monophonic source at the node origin. An implementation with per-mode acoustic transfer data MUST replace the compact-source radiation factor in [Synthesis](#synthesis).
...
- The 2π*f*ₙ factor represents compact-source radiation. A mode's impulse response *a*ₙ *e*^(−*d*ₙ*t*) sin(2π*f*ₙ*t*) is surface velocity, one factor of 2π*f*ₙ above displacement. Per-mode acoustic transfer data replaces this factor.
...
[Synthesis](#synthesis) leaves the absolute level implementation-defined. An implementation providing a physical level states the output quantity and reference distance.
...
with ρ₀ and *c*₀ the density and sound speed of the medium and σ the radiation efficiency at the mode's own frequency. σ approaches one for a body large against the wavelength and falls off as (*ka*)²/(1 + (*ka*)²) below that, with *a* the radius of a disc of area *A*. The same solution scales the [acceleration noise](#acceleration-noise), so the click and the modes arrive in the ratio the physics sets.

An implementation SHOULD attenuate each body's output by *r*₀/max(*d*, *r*₀), with *d* the distance from the listener to the body's node origin. Far-field pressure falls as 1/*d*, and the clamp inside *r*₀ prevents an unbounded sample when a listener passes through a body. This is the complete propagation model defined here. Directivity, occlusion, and reverberation remain out of scope ([Scope and Exclusions](#scope-and-exclusions)); implementations may apply this attenuation as their distance term or replace it.

The host chooses the listener position. A viewer without an explicit listener SHOULD use the active camera so audible and visible perspectives coincide. Attenuation scales only the instance output. Mode selection MUST NOT depend on listener position.

## Sustained Contact

*This section is under active development and subject to change. An implementation may support only modal models, acoustic materials, impact excitation, synthesis, and acceleration noise.*
...
| **shortWavelength** | `number` | Shortest wavelength *λ*<sub>s</sub> in the spectrum, in meters: the fine end of the described band, with *ℓ* as its coarse end. | No, default: *ℓ*/16 |
...
σ and *ℓ* set the characteristic surface slope, proportional to σ/*ℓ*. *p* sets the balance of fine and coarse texture; more negative values sound smoother. [Authoring Notes](#authoring-notes) provides representative finishes.
...
An implementation SHOULD represent roughness below *λ*<sub>s</sub> as compliance in series with each bearing contact. Pastewka et al. 2013 Appendix B gives this term in closed form. Inside a patch of width *w*, the interface uses γ times the root-mean-square height of components shorter than *w*, with γ ≈ 0.4.
...
`profile` stores a measured height track sampled uniformly at `sampleSpacing` relative to the mean plane. Values SHOULD have zero mean. Implementations traverse it cyclically, so authors SHOULD provide stationary statistics and continuous endpoints.
...
Visible millimeter-scale features such as tiling, grout, corrugation, knurling, tread, and grain belong to the mesoscale. They commonly appear in normal maps and dominate rolling sound. The finish between those features is microscale. Without `normalTexture`, visible structure contributes no acoustic relief because collision geometry omits it and the statistical parameters describe finer scales.
...
These are illustrative starting points rather than measurements. The one anchored figure is the machined row: fractal dimensions between 1.17 and 1.39 are reported for various machined surfaces at a length scale of 10⁻⁶ m, in the surface-metrology literature surveyed by van den Doel et al. 2001. The *H* = 0.7 that row implies sits in the band reported for surfaces from atomic to geological scales, typically 0.7 to 0.9 (Jacobs et al. 2017). The fractal model is valid over a wide but finite band. Fitting the power spectrum of a scrape on smooth plastic, the same work found it valid to about 1 kHz.

Surfaces may be characterized from a measured height track or a scrape recording. A stylus instrument or atomic force microscope provides `profile` at micrometer resolution; Agarwal et al. 2021 use 5.6 µm. These methods agree over shared scales. Optical methods lose resolution near 1 µm and are unsuitable for quantitative roughness spectra (Rodriguez et al. 2024). A contact at speed *v* consumes *v*/`sampleSpacing` samples per second. At 5 µm spacing and 10 cm/s, a one-second loop needs roughly 20,000 samples. A scrape recording provides *p* through a linear spectral fit; σ and *ℓ* can then be matched against the recording.
...
The *contact force* is the physical force between the bodies. Its component along **n̂** supports the static load and is non-negative; the remaining component lies in the contact plane. The modal *excitation* **F**(*t*) is the zero-mean fluctuation of that force and may have either sign in every component.
...
- The contact force MUST be non-negative: a separated contact applies no force. This nonlinearity produces micro-collisions and chatter.
...
- A contact at rest MUST produce no output. With zero slip and sweep speeds and constant *N*, the excitation is zero regardless of load.
...
- Loudness SHOULD increase with normal force and surface traversal rate. Every component scales with load: the normal component through contact stiffness, the geometric tangential component through projection onto a tilted surface, and the frictional component through the Coulomb bound *μN*. Speed scales the traversal spectrum, increasing rolling loudness even at zero slip.
- Roughness with wavelengths shorter than the load-bearing region SHOULD be attenuated. This **contact filter**, standard in rolling-noise prediction since Remington 1987, acts on spatial wavelength: a region of width *w* cannot resolve wavelengths below roughly *w*. Its audible cutoff is near *v*<sub>sweep</sub>/*w*, with measured filters rolling off gradually below that frequency.
- The load-bearing region is the real asperity contact area, which is orders of magnitude smaller than the confining region. The microscale finish SHOULD be filtered over the real contact area's width. Longer relief SHOULD be filtered over the smaller of the Hertz patch and shared face polygon, reduced further by [waviness](#acoustic-surfaces). [Appendix B](#appendix-b-reference-contact-force-model) provides one expression for all regimes.
...
`normalTexture` is the only surface quantity affected by node scale. Its features use texture coordinates, so their physical size follows the primitive parameterization and global node scale. Implementations MUST derive texel size in meters from the global scale at the contact. Two differently scaled instances of one mesh therefore use different relief dimensions.
...
- A rigid body is one elastic object, so at most one node in the body and collider hierarchy MUST define a modal model. Each collider MAY define its own acoustic surface.
- Local surface geometry used by a contact model, such as curvature, SHOULD come from the contacted collider node's mesh or its nearest ancestor with a mesh.
- Collider geometry and acoustic surfaces resolve independently. A collider with a mesh supplies that geometry regardless of whether it also defines a surface.
...
**KHR_audio_emitter / KHR_audio_graph** (proposals): This extension generates source signals and does not define emitters, listeners, or spatialization. When a node with a modal model also has an audio emitter, the synthesized signal SHOULD use that emitter as its source.

An acoustic material is distinct from physics materials for friction and restitution and from render materials. A node may define all three.
...
- **Sound propagation and spatialization**: occlusion, reverberation, and listener modeling belong to audio-emitter and platform layers. [Radiation and Distance](#radiation-and-distance) defines the inverse-distance falloff required for a physical output level.
...
- **Anisotropic and spatially varying microscale finish**: `roughness`, `correlationLength`, and `spectralSlope` are isotropic and uniform over the body. Sub-visible directional finishes therefore sound identical along and across the grain. `normalTexture` provides mesoscale variation and directionality along the contact path.
- **Position-indexed traversal of the microscale finish**: `profile` and σ, *ℓ*, *p* describe a one-dimensional realization indexed by distance. Retracing therefore reads a new realization. A two-dimensional field can preserve retracing over a bounded contact excursion, as in Agarwal et al. 2021. For *T* seconds of non-repeating output, unbounded sliding requires approximately *T* *f*<sub>Nyquist</sub> samples in one dimension and its square in two dimensions. Procedural evaluation avoids field storage and synthesizes only the octaves resolved by current speed and load. One-dimensional projections that preserve retracing introduce directional dependence. The mesoscale `normalTexture` already provides a two-dimensional field sampled at contact position **p**.
...
Rayleigh damping is a two-parameter special case and cannot represent every material: its stiffness term fixes the exponent relating damping to frequency, while measured exponents vary by material (Sterling & Lin 2016). The same study found no perceived realism improvement from its power-law model over Rayleigh damping.

Per-mode `decayRates` represent any damping distribution exactly for interchange. Differentiable modal renderers also fit a damping factor per mode rather than deriving one from a material (Jin et al. 2024). Render-time Rayleigh damping applies only when re-deriving damping after uniform rescaling. Authors of fitted decay rates SHOULD omit `alpha` and `beta` to preserve the measured values.
...
A contact has either a load-created area for a point or edge, or a geometry-fixed nominal area for two faces. **The regimes differ only in the confining region.** One force law and one area law cover both.

The elastic compliance is shared:

1/*E*\* = (1 − ν₁²)/*E*₁ + (1 − ν₂²)/*E*₂

**Load-set contact, where the physics reports no nominal area.** Hertz theory for normal contact between elastic solids (Johnson 1985) uses κ, the mean curvature of each body at the contact from the contacted collider geometry:
...
Its limits have closed forms. A patch that remains below its cap gives Hertz's τ = 2.868 (*m*\*²/(*E*\*² *R*\* *v*))^(1/5), shorter for a harder or faster impact. The flat-face limit gives τ = π √(*m*\*/*k*) at constant *k* = 2 *E*\* √(*A*₀<sub>poly</sub>/π), independent of impact speed. Face contact is stiffer than point contact and therefore shorter and brighter at equal mass. A patch that reaches its cap during collision lasts longer than either limit.

**Real contact area.** In both regimes, contacting asperities support the load at a pressure independent of load and area. Let *A*₀ be the smaller of the Hertz patch π*a*² and the shared polygon. Pastewka and Robbins 2016 give the area across the full load range:
...
|∇*h*|<sub>rms</sub> is the root-mean-square surface gradient. For an isotropic surface, it is √2 times the one-dimensional track gradient. **Measure it from the track rather than deriving it from roughness, correlation length, and spectral exponent.** A closed form over the band overshoots by up to a factor of 1.7 because the flat spectral region contributes height but no slope.
...
For surfaces with waviness, *A*₀ for a geometry-set contact is the waviness-limited bearing area. Evaluate the same area law one scale higher, using mesoscale gradient *W*/*L*<sub>W</sub> and the shared polygon. Resolve the finish inside that region, corresponding to two discrete Persson magnification steps.

Then, each sample:

- Maintain one read position per surface and advance it by the distance that surface travels through the contact. In samples, advance by sweep speed divided by track sample spacing: `sampleSpacing` for a stored profile or a synthesis parameter otherwise.
- Form each surface relief *h*ᵢ = *h*<sub>meso,ᵢ</sub> + σᵢ *h*<sub>micro,ᵢ</sub>, then form gap relief *h* = *h*₁ + *h*₂. The microscale term is the track sample at the read position. The mesoscale term integrates the `normalTexture` slope along the path and is zero without a texture. Use a leaky integrator because a sampled normal map is not exactly a gradient field. The tangential term uses each surface slope directly. Equilibrium subtraction removes the constant contribution from a surface with zero sweep.
...
- Form approach δ = δ₀ + *h* − *u*, where δ₀ is the equilibrium separation and *u* is the modal displacement from [Vibrational Coupling](#vibrational-coupling), or zero without coupling. Clamp a single spring at zero. For a bed, retain signed approach and clamp each spot independently. This per-spot clamp preserves the separation nonlinearity averaged out by a mean pressure-separation law.
...
- Form tangential force from two parameter-free components. The geometric component projects normal force onto the locally tilted surface: *f*<sub>t,geo,ᵢ</sub> = *f*<sub>n</sub> ∂*h*ᵢ. The frictional component applies Coulomb traction to the force fluctuation: *f*<sub>t,fric</sub> = *μ*(*f*<sub>n</sub> − *N*). Remove the geometric component's local mean so both components are equilibrium fluctuations. Agarwal et al. 2021 use *f*<sub>h</sub> = β₁|**v** · ∇*S*|^β₂ with β₁ = 0.05 and β₂ = 1, where **v** is surface traversal velocity. The Hunt and Crossley damping factor expands the geometric projection into an elastic term and a velocity-dependent term. Matching their expression identifies β₁ with *f*<sub>n</sub> *c*<sub>d</sub> ∂*h*, with units N·s·m⁻¹. A fitted β₁ absorbs load, dissipation, and surface slope. Projecting the complete contact force provides both terms without fitted parameters.
...
- Soft-limit the roughness-driven part of *f*<sub>n</sub> with a knee near one hundred times *N* and match the linear-region slope. Rough-contact fluctuations can exceed the load by large factors, so a knee at *N* compresses the modeled roughness. Cap total force at a much higher multiple because both force laws grow without bound in δ and changing geometry can produce an out-of-regime δ. Agarwal et al. instead limit trajectory curvature with a normal-force-dependent tanh nonlinearity.
