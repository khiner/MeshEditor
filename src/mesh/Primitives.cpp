...
namespace primitive {
MeshData CreateMesh(const Plane &p) {
...
MeshData CreateMesh(const Circle &p) {
...
MeshData CreateMesh(const Cuboid &p) {
...
MeshData CreateMesh(const IcoSphere &p) {
    ...
    for (auto &v : vertices) v = Normalize(v);
    ...
    auto AddMidVertex = [&](uint32_t p1, uint32_t p2) -> uint32_t {
        ...
        vertices.emplace_back(Normalize((vertices[p1] + vertices[p2]) / 2.f));
...
MeshData CreateMesh(const UVSphere &p) {
...
MeshData CreateMesh(const Torus &p) {
...
MeshData CreateMesh(const Cylinder &p) {
...
MeshData CreateMesh(const Cone &p) {
    const auto [radius, height, slices] = p;
    std::vector vertices =
...
MeshData CreateMesh(const PrimitiveShape &shape) {
    return std::visit([](const auto &s) -> MeshData { return CreateMesh(s); }, shape);
}

BoneOctahedronData BoneOctahedron(float length) {
...
BoneSphereData BoneSphereDisc(float radius, uint32_t segments) {
    std::vector<vec3> positions;
