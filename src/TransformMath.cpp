...
Transform ToTransform(const mat4 &m) {
    ...
    vec3 scale{Length(col0), Length(col1), Length(col2)};
    ...
    if (Determinant(mat3{col0, col1, col2}) < 0) scale.x = -scale.x;
    // Use per-axis thresholds because determinant thresholds reject quantized per-axis scales near 3.6e-6.
    ...
    const quat rotation = ToQuat(mat3{col0, col1, col2});
