...
// Encode tightly-packed RGBA8 pixels. PNG and WebP are lossless. JPEG honors `quality` (1–100).
...
std::expected<std::vector<std::byte>, std::string> EncodeImageWebpRgba8(std::span<const std::byte> rgba8, uint32_t width, uint32_t height, std::string_view name);
