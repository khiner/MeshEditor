...
constant uint OverlayJobGroupLines = uint(OverlayDispatch::LineGroupLines);
constant uint LightRangeSegments = uint(OverlayDispatch::LightRangeSegments);
constant uint SpotConeSegments = uint(OverlayDispatch::SpotConeSegments);
constant float SpotConeDepth = 2.0f;
constant uint ColliderCircleSegments = uint(OverlayDispatch::ColliderCircleSegments);
...
inline bool IsColliderKind(ExtrasLineKind kind) { return kind >= ExtrasLineKind::ColliderBox; }
...
inline float3 ColliderWirePoint(OverlayJob job, uint line, uint end) {
    ...
    if (job.ExtrasKind == ExtrasLineKind::ColliderBox) {
    ...
    if (job.ExtrasKind == ExtrasLineKind::ColliderSphere) {
    ...
    const uint profile_lines = job.ExtrasKind == ExtrasLineKind::ColliderCapsule ? ColliderCapLines : ColliderCircleSegments;
    ...
    if (job.ExtrasKind == ExtrasLineKind::ColliderCylinder) return ColliderRingPoint(profile_line, end, radius, height);
...
inline ExtrasLineVertex ExtrasLineVertexAt(OverlayJob job, uint line, uint end) {
    ...
    const bool rings_range = job.ExtrasKind != ExtrasLineKind::LightDirectional && range > 0.0f;
    ...
    if (job.ExtrasKind == ExtrasLineKind::LightDirectional) {
        ...
        local -= ray_count * 2u;
    } else if (job.ExtrasKind == ExtrasLineKind::LightSpot) {
...
inline float4 OverlayJobClipVertex(
    ...
    if (job.Kind == OverlayJobKind::Extras) {
        float4 clip;
    ...
    if (job.Kind == OverlayJobKind::TetWire) clip.z -= NdcOffsetFactor(scene);
    return clip;
}
