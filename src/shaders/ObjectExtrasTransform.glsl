...
uint GetVertexClass(DrawData draw, uint idx) {
    if (draw.VertexClassOffset != INVALID_OFFSET) {
        return uint(VertexClassBuffers[SceneViewUBO.VertexClassSlot].Classes[draw.VertexClassOffset + idx]);
    }
    return VCLASS_NONE;
}
...
vec3 ObjectExtrasWorldPos(DrawData draw, Vertex vert, WorldTransform world, uint idx) {
    ...
    if (vertex_class == VCLASS_SPOT_CONE) {
        ...
        const vec3 world_pos = vec3(M * vec4(vert.Position, 1.0));
        ...
        const vec3 p0 = vec3(M * vec4(vert.Position.xy * slope.x + perp * slope.y, vert.Position.z, 1.0));
        const vec3 p1 = vec3(M * vec4(vert.Position.xy * slope.x - perp * slope.y, vert.Position.z, 1.0));
    ...
    return trs_transform_point(world, vert.Position);
}
