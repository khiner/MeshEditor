...
constant float DISC_RADIUS = 0.5641895835477563f * 1.05f; // Uses M_1_SQRTPI * 1.05.
constant float LINE_SMOOTH_START = 0.5f - DISC_RADIUS;
constant float LINE_SMOOTH_END = 0.5f + DISC_RADIUS;

// Sample the center and four orthogonal neighbors to cover the line-filter radius.
constant float2 CompositeOffsets[5] = {
    float2(0, 0), float2(-1, 0), float2(1, 0), float2(0, -1), float2(0, 1)
};

// Returns the antialiased overlay sample with the strongest neighboring line coverage.
inline float4 ResolveOverlay(const thread Scene &scene, constant ViewportCompositePushConstants &pc, float2 uv) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        if (n_line.a < 0.5f) continue;

        const float2 perp = decode_line_dir(n_line.xy);
        // Measure the current pixel against the line represented by sample i.
    ...
    // Scale premultiplied color and alpha together when applying neighboring line coverage.
