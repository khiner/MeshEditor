...
void main() {
    ...
    if (draw.ObjectIdSlot != INVALID_SLOT) {
        ...
        if (draw.FaceFirstTriOffset.Slot != INVALID_SLOT && face_id != 0u) {
            ...
            const uint first_tri = ObjectIdBuffers[draw.FaceFirstTriOffset.Slot].Ids[draw.FaceFirstTriOffset.Offset + face_id - 1u];
            const uint base = draw.IndexOffset.Offset + first_tri * 3u;
            const uint i0 = IndexBuffers[draw.IndexOffset.Slot].Indices[base];
            const uint i1 = IndexBuffers[draw.IndexOffset.Slot].Indices[base + 1u];
            const uint i2 = IndexBuffers[draw.IndexOffset.Slot].Indices[base + 2u];
        ...
        if (draw.ElementStateOffset.Slot != INVALID_SLOT && face_id != 0u) {
            element_state = uint(ElementStateBuffers[draw.ElementStateOffset.Slot].States[draw.ElementStateOffset.Offset + face_id - 1u]);
        }
    } else if (draw.ElementStateOffset.Slot != INVALID_SLOT) {
        element_state = uint(ElementStateBuffers[draw.ElementStateOffset.Slot].States[draw.ElementStateOffset.Offset + gl_VertexIndex]);
    }
    ...
    const vec4 base_color = draw.ObjectIdSlot != INVALID_SLOT ? object_base_color :
        draw.ElementStateOffset.Slot != INVALID_SLOT ? edge_color :
    ...
    const bool is_edge_draw = !is_face_draw && draw.ElementStateOffset.Slot != INVALID_SLOT;
