...
inline uint MeshletVertexId(const thread Scene &scene, DrawData draw, uint packed_vertex) {
    return scene.Indices(draw.IndexSlotOffset.Slot)[draw.IndexSlotOffset.Offset + (packed_vertex & MeshletGeometryEncoding_CornerMask)];
}
