...
bool should_apply_pending_transform(DrawData draw, uint idx) {
    ...
    if (pc.TransformVertexStateSlot != INVALID_SLOT) {
        if (draw.PendingLocalTransform.Slot == INVALID_SLOT) return false;
    ...
    if (SceneViewUBO.InteractionMode == InteractionModeEdit || draw.InstanceState.Slot == INVALID_SLOT) return false;
    const uint instance_state = uint(InstanceStateBuffers[draw.InstanceState.Slot].States[draw.InstanceState.Offset + draw.FirstInstance]);
