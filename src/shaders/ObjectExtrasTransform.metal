...
inline float3 ObjectExtrasWorldPos(const thread SceneT<SetT> &scene, Transform world, float3 vert_pos, uint vertex_class) {
    ...
    if (vertex_class == VCLASS_BILLBOARD || vertex_class == VCLASS_SCREENSPACE) {
        ...
        // Screenspace: maintain constant pixel size regardless of distance and zoom.
        // ScreenPixelScale > 0 means perspective (multiply by distance),
        // ScreenPixelScale < 0 means orthographic (use the absolute value directly).
        float scale = 1.0f;
    ...
    if (vertex_class == VCLASS_SPOT_CONE) {
        ...
        // Only spoke edges at facing transitions are visible, and others collapse to the apex.
        const float4x4 M = trs_to_mat4(world);
        const float3 world_pos = (M * float4(vert_pos, 1.0f)).xyz;

        // Adjacent points on the cone base circle, by 2D rotation.
    ...
    if (vertex_class == VCLASS_GROUNDPOINT) {
        // Ground-plane projection. vert_pos.y interpolates between ground (0) and object origin (1).
        // vert_pos.xz are screenspace-scaled offsets, for a constant-pixel-size ground diamond.
