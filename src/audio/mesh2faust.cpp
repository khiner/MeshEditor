...
m2f::ModalModel m2f::mesh2modal(TetMesh *volumetricMesh, MaterialProperties material, CommonArguments args) {
    if (args.debugMode) std::cout << "Creating and computing mass matrix\n";

    SparseMatrix *massMatrix;
    GenerateMassMatrix::computeMassMatrix(volumetricMesh, &massMatrix, true);

    if (args.debugMode) std::cout << "Creating and computing stiffness matrix\n";
...
m2f::ModalModel m2f::mesh2modal(
    ...
    if (args.debugMode) {
        ...
        std::cout << femNModes << " modes will be computed for FEM analysis.\n\n";
    }

    using OpType = Spectra::SymShiftInvert<double, Eigen::Sparse, Eigen::Sparse>;
    using BOpType = Spectra::SparseSymMatProd<double>;

    const int convergenceRatio = std::min(std::max(2 * femNModes + 1, 20), numVertices * vertexDim);
    ...
    if (eigs.info() != Spectra::CompInfo::Successful) {
        if (args.debugMode) std::cout << "Could not compute eigenvalues.\n";
        return {};
    }
    ...
    if (args.debugMode) std::cout << "Computing modes ...\n\n";
    ...
    for (size_t exPos = 0; exPos < size_t(args.nExPos); ++exPos) { // For each excitation position
...
std::string m2f::modal2faust(const ModalModel &model, DspGenArguments args) {
    ...
    for (size_t exPos = 0; exPos < gains.size(); ++exPos) {
        for (size_t mode = 0; mode < gains[exPos].size(); ++mode) {
