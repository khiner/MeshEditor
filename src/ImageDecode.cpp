...
std::string StbiFailureReasonOrUnknown() {
    ...
    return reason ? std::string{reason} : std::string{"unknown stb_image error"};
}
...
std::expected<DecodedImageF32, std::string> DecodeImageFileRgba32f(const std::filesystem::path &path, std::string_view image_name) {
    const std::string label = image_name.empty() ? path.string() : std::string{image_name};
    ...
    if (auto valid = ValidateDimensions(width, height, label); !valid) {
        stbi_image_free(pixels);
        return std::unexpected{std::move(valid.error())};
    }
