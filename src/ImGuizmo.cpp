...
struct vec_t {
    ...
    vec_t(float x, float y, float z, float w = 0) : _v(x, y, z, w) {}
    vec_t(ImVec2 v) : _v(v.x, v.y, 0, 0) {}
...
constexpr float GetSegmentLengthClipSpace(const vec_t &start, const vec_t &end, const bool local_coords = false) {
    ...
    if (fabsf(segment_start._v.w) > FLT_EPSILON) segment_start._v /= segment_start._v.w;

    auto segment_end = end;
    segment_end.TransformPoint(mvp);
    // check for axis aligned with camera direction
    if (fabsf(segment_end._v.w) > FLT_EPSILON) segment_end._v /= segment_end._v.w;
...
constexpr float GetParallelogram(const vec_t &p0, const vec_t &pa, const vec_t &pb) {
    ...
    for (uint32_t i = 0; i < 3; i++) {
        ...
        // check for axis aligned with camera direction
        if (fabsf(pts[i]._v.w) > FLT_EPSILON) pts[i]._v /= pts[i]._v.w;
    }
    auto seg_a = pts[1]._v - pts[0]._v;
    seg_a.y /= g.DisplayRatio;

    auto seg_b = pts[2]._v - pts[0]._v;
...
constexpr vec_t PointOnSegment(const vec_t &p, const vec_t &vert_p1, const vec_t &vert_p2) {
    const auto v = glm::normalize(vert_p2._v - vert_p1._v);
    const float t = glm::dot(vec3{v}, vec3{p._v - vert_p1._v});
    if (t < 0.f) return vert_p1;
    if (t > glm::length(vert_p2._v - vert_p1._v)) return vert_p2;
    return vert_p1 + v * t;
}
...
void ComputeContext(const mat4 &view, const mat4 &proj, mat4 &m, MODE mode) {
    ...
    g.ScreenSquareCenter = WorldToPos({0, 0, 0}, g.MVP);
...
void ComputeTripodAxisAndVisibility(const int axis_i, vec_t &dir_axis, vec_t &dir_plane_x, vec_t &dir_plane_y, bool &below_axis_limit, bool &below_plane_limit, const bool local_coords = false) {
    dir_axis._v = {DirUnary[axis_i], 0};
    dir_plane_x._v = {DirUnary[(axis_i + 1) % 3], 0};
    dir_plane_y._v = {DirUnary[(axis_i + 2) % 3], 0};
    ...
    if (g.Using && (g.ActualID == -1 || g.ActualID == g.EditingID)) {
        ...
        dir_axis._v *= g.AxisFactor[axis_i];
        dir_plane_x._v *= g.AxisFactor[(axis_i + 1) % 3];
        dir_plane_y._v *= g.AxisFactor[(axis_i + 2) % 3];
    } else {
        const vec_t origin{0, 0, 0};
        ...
        dir_axis._v *= mul_axis;
        dir_plane_x._v *= mul_axis_x;
        dir_plane_y._v *= mul_axis_y;
...
constexpr float IntersectRayPlane(const vec_t &origin, const vec_t &dir, const vec_t &plan) {
    const float num = glm::dot(vec3{plan._v}, vec3{origin._v}) - plan._v.w;
    const float den = glm::dot(vec3{plan._v}, vec3{dir._v});
...
void DrawRotationGizmo(OPERATION op, int type) {
    ...
    for (int axis = 0; axis < 3; axis++) {
        ...
        for (int i = 0; i < circle_mul * HalfCircleSegmentCount + 1; i++) {
            ...
            const vec_t axis_pos{cosf(ng), sinf(ng), 0};
            const auto pos = vec_t{axis_pos[axis], axis_pos[(axis + 1) % 3], axis_pos[(axis + 2) % 3]} * g.ScreenFactor * RotationDisplayScale;
...
int GetScaleType(OPERATION op) {
    ...
    if (float dist = glm::length(delta_screen._v); dist >= 17.0f && dist < 23.0f && Contains(op, SCALEU)) type = MT_SCALE_XYZ;
...
int GetRotateType(OPERATION op) {
    ...
    const auto mouse_pos = ImGui::GetIO().MousePos;
    const vec_t delta_screen{{mouse_pos.x - g.ScreenSquareCenter.x, mouse_pos.y - g.ScreenSquareCenter.y, 0, 0}};
    const auto dist = glm::length(delta_screen._v);
...
int GetMoveType(OPERATION op, vec_t *hit_proportion = nullptr) {
    ...
    const vec_t pos_screen{io.MousePos - ImVec2{g.X, g.Y}};
    for (int i = 0; i < 3 && type == MT_NONE; i++) {
        vec_t dir_plane_x, dir_plane_y, dir_axis;
        bool below_axis_limit, below_plane_limit;
        ComputeTripodAxisAndVisibility(i, dir_axis, dir_plane_x, dir_plane_y, below_axis_limit, below_plane_limit);
        ...
        if (hit_proportion != nullptr) *hit_proportion = {dx, dy, 0};
    }
    return type;
}
...
bool HandleScale(mat4 &m, OPERATION op, int &type, const float *snap) {
    ...
    if (g.Using && (g.ActualID == -1 || g.ActualID == g.EditingID) && IsScaleType(g.CurrentOp)) {
        ...
        if (g.CurrentOp >= MT_SCALE_X && g.CurrentOp <= MT_SCALE_Z) {
            ...
            g.Scale._v = vec4{std::max(1.f + scale_delta, 0.001f)};
        }
