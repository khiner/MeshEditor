...
layout(binding = 0) uniform sampler2D Tex; // Assumes {Depth, ObjectID}
...
void main() {
    const vec2 this_value = texture(Tex, TexCoord).xy; // {Depth, ObjectID}
    ...
    for (int i = -1; i <= 1; i++) {
        for (int j = -1; j <= 1; j++) {
            if (i == 0 && j == 0) continue;
            const vec2 neighbor_value = texture(Tex, TexCoord + vec2(i, j) * neighborhood_half_extent).xy;
